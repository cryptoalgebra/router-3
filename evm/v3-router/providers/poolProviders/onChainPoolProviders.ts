...
export const getV3PoolsWithoutTicksOnChain = createOnChainPoolFactory<
  V3Pool,
  V3PoolMeta
>({
  abi: algebraPoolABI,
  getPossiblePoolMetas: ([currencyA, currencyB]) => {
    return [CUSTOM_POOL_BASE ,CUSTOM_POOL_DEPLOYER_LIMIT_ORDER].map(
      (deployer) => ({
  ...
  buildPool: (
    { currencyA, currencyB, address, deployer },
    [liquidity, globalState]
  ) => {
    if (!globalState) {
      return null;
    }
...
interface OnChainPoolFactoryParams<
  TPool extends Pool,
  TPoolMeta extends PoolMeta,
...
> {
  ...
  buildPoolInfoCalls: (
    poolAddress: Address
  ) => Omit<ContractFunctionConfig<TAbi>, "abi">[];
  ...
  return async function poolFactory(
    ...
    const results = await client.multicall({
      ...
      blockNumber: blockNumber
        ? BigInt(Number(BigInt(blockNumber)))
        : undefined,
    });
